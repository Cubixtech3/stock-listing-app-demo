...
const BottomSheet: React.FC<{ 
  part: PartItem | null; 
  onClose: () => void;
  onInquirySubmit: (inquiry: Omit<Inquiry, 'id' | 'timestamp' | 'status'>) => void;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = `*📦 NEW INQUIRY - ${businessName}*\n` +
      `----------------------------------\n` +
      `*Part:* ${part.name}\n` +
      `*OEM:* ${part.oem}\n` +
      `*Brand:* ${part.brand}\n` +
              ...
              {!showForm ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6">
                    <h3 className="text-sm font-bold text-slate-900 mb-1">Requesting: {part.name}</h3>
                    <p className="text-xs text-slate-500">Quantity: {quantity} units • Total: ${(part.price * quantity).toFixed(2)}</p>
                  </div>
...
const AdminDashboard: React.FC<{ 
  ...
  const handleReply = (inquiry: Inquiry) => {
    const message = `*✅ ORDER ACCEPTED - AutoPart Pro*\n` +
      `----------------------------------\n` +
  ...
  const handleReject = (inquiry: Inquiry) => {
    const message = `*❌ INQUIRY UPDATE - AutoPart Pro*\n` +
      `----------------------------------\n` +
      `Hello *${inquiry.customerName}*,\n\n` +
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Inquiry Manager</h2>
          <p className="text-xs text-slate-500 font-medium">Manage wholesale requests and WhatsApp replies</p>
        </div>
      ...
      {inquiries.length === 0 ? (
        <div className="py-32 flex flex-col items-center justify-center text-slate-300 text-center">
          ...
          <p className="text-sm font-bold text-slate-400">No pending inquiries</p>
          <p className="text-[10px] uppercase tracking-widest mt-1">Waiting for customers...</p>
        </div>
      ) : (
        <div className="space-y-4">
          {inquiries.sort((a, b) => b.timestamp - a.timestamp).map((inquiry) => (
            <motion.div 
              layout
            ...
            >
              ...
              <div className={`h-1.5 w-full ${
                inquiry.status === 'accepted' ? 'bg-emerald-500' : 
                inquiry.status === 'rejected' ? 'bg-rose-500' : 'bg-amber-400'
              }`} />
              ...
              <div className="p-5">
                <div className="flex justify-between items-start mb-4">
                  <div className="min-w-0 flex-grow">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] font-mono text-indigo-600 font-bold">{inquiry.oem}</span>
                    </div>
                    <h3 className="font-bold text-slate-900 text-base truncate">{inquiry.partName}</h3>
                  </div>
                  <div className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider ${
                    inquiry.status === 'accepted' ? 'bg-emerald-50 text-emerald-600' : 
                    inquiry.status === 'rejected' ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'
                  }`}>
                    {inquiry.status}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="space-y-1">
                    <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Customer</span>
                    <div className="flex items-center gap-2">
                      <User size={12} className="text-slate-400" />
                      <p className="text-xs font-bold text-slate-700 truncate">{inquiry.customerName}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Phone size={12} className="text-slate-400" />
                      <p className="text-[11px] text-slate-500 font-medium">{inquiry.customerPhone}</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Order Details</span>
                    <div className="flex items-center gap-2">
                      <ShoppingBag size={12} className="text-slate-400" />
                      <p className="text-xs font-bold text-slate-700">Qty: {inquiry.quantity} Units</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock size={12} className="text-slate-400" />
                      <p className="text-[10px] text-slate-500 font-medium">{new Date(inquiry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {new Date(inquiry.timestamp).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
...
export default function App() {
  ...
  const handleInquirySubmit = (data: Omit<Inquiry, 'id' | 'timestamp' | 'status'>) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {viewMode === 'admin' ? <ShieldCheck size={14} /> : <LayoutDashboard size={14} />}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 px-6 py-3 flex justify-between items-center z-40">
        {[
          { icon: ShoppingBag, label: 'Store', active: viewMode === 'user', onClick: () => setViewMode('user') },
          { icon: LayoutDashboard, label: 'Admin', active: viewMode === 'admin', onClick: () => setViewMode('admin') },
          { icon: MessageCircle, label: 'Orders', active: false, onClick: () => {} },
